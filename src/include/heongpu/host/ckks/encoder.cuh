...
namespace heongpu
{
    ...
    template <> class HEEncoder<Scheme::CKKS>
    {
      ...
      public:
        ...
        {
            if ((scale <= 0) ||
                (static_cast<int>(log2(scale)) >= context_->total_coeff_bit_count))
            {
                throw std::invalid_argument("Scale out of bounds");
            }
                ...
                [&](Plaintext<Scheme::CKKS> plain_ringt_)
                {
                    output_storage_manager(
                        ...
                        [&](Plaintext<Scheme::CKKS>& plain_pt_)
                        {
                            ...
                            plain_pt_.plain_size_ = context_->n * current_decomp_count;
                            plain_pt_.scheme_ = context_->scheme_;
