...
#pragma unroll
        ...
        Data64 m_tilde_div_2 = m_tilde.value >> 1;
        Data64 r_m_tilde = OPERATOR_GPU_64::mult(
            temp2[obase_size], inv_prod_q_mod_m_tilde[0], m_tilde);
        r_m_tilde = m_tilde.value - r_m_tilde;
...
#pragma unroll
    ...
    __global__ void cross_multiplication(Data64* in1, Data64* in2, Data64* out,
                                         Modulus64* modulus, int n_power,
                                         int decomp_size, int Q_size, int level)
    {
        int idx = blockIdx.x * blockDim.x + threadIdx.x; // ring size
        int idy = blockIdx.y; // decomp size + bsk size
        ...
        int mod_location = (idy < Q_size) ? idy : (idy + level);

        Data64 out_0 = OPERATOR_GPU_64::mult(ct0_0, ct1_0, modulus[mod_location]);
        Data64 out_1_0 = OPERATOR_GPU_64::mult(ct0_0, ct1_1, modulus[mod_location]);
        Data64 out_1_1 = OPERATOR_GPU_64::mult(ct0_1, ct1_0, modulus[mod_location]);
        Data64 out_2 = OPERATOR_GPU_64::mult(ct0_1, ct1_1, modulus[mod_location]);
        Data64 out_1 = OPERATOR_GPU_64::add(out_1_0, out_1_1, modulus[mod_location]);
...
#pragma unroll
        ...
        alpha_sk = OPERATOR_GPU_64::add(alpha_sk, temp4[ibase_size],
                                        obase[obase_size - 1]);
        alpha_sk = OPERATOR_GPU_64::mult(alpha_sk, inv_prod_B_mod_m_sk[0],
                                         obase[obase_size - 1]);
