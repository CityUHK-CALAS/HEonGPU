...
namespace heongpu
{
                                        ...
                                        HEEncoder<Scheme::BFV>& encoder)
    {
        ...
        // Initialize prime location vectors for GPU_NTT_Modulus_Ordered_Inplace
        ...
        for (int i = 0; i < context_->Q_size; i++)
        {
            ...
            counter--;
            for (int j = 0; j < context_->bsk_modulus[i]; j++)
            {
                prime_bsk_loc.push_back(context_->Q_size + j);
            }
        }
                                               ...
                                               const ExecutionOptions& options)
    {
        ...
        if (input1.memory_size() <
            ...
            input2.memory_size() <
                (cipher_size * context_->n * current_decomp_count)) // @company CipherFlow
        {
                        ...
                        output_storage_manager(
                            ...
                            [&](Ciphertext<Scheme::BFV>& output_)
                            {
                                ...
                                addition<<<dim3((context_->n >> 8),
                                                current_decomp_count, cipher_size), // @company CipherFlow
            ...
            [&](Ciphertext<Scheme::BFV>& input1_)
            {
                output_storage_manager(
                    ...
                    [&](Ciphertext<Scheme::BFV>& output_)
                    {
                        ...
                        negation<<<dim3((context_->n >> 8), current_decomp_count, // @company CipherFlow
                                        cipher_size),
                                   256, 0, options.stream_>>>(
        ...
        Ciphertext<Scheme::BFV>& output, const cudaStream_t stream)
    {
        ...
        if (input1.memory_size() < (cipher_size * context_->n * current_decomp_count)) // @company CipherFlow
        {
            throw std::invalid_argument("Invalid Ciphertexts size!");
        }
        ...
        const cudaStream_t stream)
    {
        ...
        addition_plain_bfv_poly_inplace<<<dim3((context_->n >> 8), current_decomp_count, 1), 256, 0,
                                          stream>>>(
            input1.data(), input2.data(), input1.data(), context_->modulus_->data(),
            context_->plain_modulus_, context_->Q_mod_t_->data() + location, context_->upper_threshold_,
            context_->coeeff_div_plainmod_->data() + location, context_->n_power); // @company CipherFlow
        HEONGPU_CUDA_CHECK(cudaGetLastError());
    }
        ...
        const cudaStream_t stream)
    {
        ...
        substraction_plain_bfv_poly_inplace<<<dim3((context_->n >> 8), current_decomp_count, 1), 256,
                                              0, stream>>>(
            input1.data(), input2.data(), input1.data(), context_->modulus_->data(),
            context_->plain_modulus_, context_->Q_mod_t_->data() + location, context_->upper_threshold_,
            context_->coeeff_div_plainmod_->data() + location, context_->n_power); // @company CipherFlow
    ...
    {   
        ...
        DeviceVector<Data64> output_memory((3 * context_->n * current_decomp_count), stream); // @company CipherFlow

        DeviceVector<Data64> temp_mul((4 * context_->n * (context_->bsk_modulus[0] + context_->Q_size)) +
                                          (3 * context_->n * (context_->bsk_modulus[0] + context_->Q_size)),
                                      stream);  // @company CipherFlow
        Data64* temp1_mul = temp_mul.data();
        Data64* temp2_mul = temp1_mul + (4 * context_->n * (context_->bsk_modulus[0] + context_->Q_size));  // @company CipherFlow
        ...
        {   
            location += counter;
            location1 += context_->bsk_modulus[i];
            location2 += context_->bsk_modulus[i] * counter;
            location3 += context_->bsk_modulus[i]-1;
            location4 += (context_->bsk_modulus[i]-1) * counter;
            location5 += counter + context_->bsk_modulus[i];
            counter--;
        }
        // @company CipherFlow end ---
        ...
        fast_convertion<<<dim3((context_->n >> 8), 4, 1), 256, 0, stream>>>(
            ...
            context_->base_Bsk_->data(), context_->m_tilde_, context_->inv_prod_q_mod_m_tilde_->data() + input1.depth_,
            context_->inv_m_tilde_mod_Bsk_->data(), context_->prod_q_mod_Bsk_->data() + location1,
            context_->base_change_matrix_Bsk_->data() + location2,
            context_->base_change_matrix_m_tilde_->data() + location,
            context_->inv_punctured_prod_mod_base_array_->data() + location, context_->n_power, current_decomp_count,
            context_->bsk_modulus[input1.depth_]); // @company CipherFlow
        HEONGPU_CUDA_CHECK(cudaGetLastError());
        ...
        gpuntt::GPU_NTT_Modulus_Ordered_Inplace(
            ...
            ((context_->bsk_modulus[input1.depth_] + current_decomp_count) * 4), (context_->bsk_modulus[input1.depth_] + current_decomp_count),
            new_prime_bsk_locations + location5); // @company CipherFlow

        cross_multiplication<<<dim3((context_->n >> 8), (context_->bsk_modulus[input1.depth_] + current_decomp_count), 1),
                               256, 0, stream>>>(
            temp1_mul, temp1_mul + (((context_->bsk_modulus[input1.depth_] + current_decomp_count) * 2) * context_->n),
            temp2_mul, context_->q_Bsk_merge_modulus_->data(), context_->n_power,
            (context_->bsk_modulus[input1.depth_] + current_decomp_count),
        ...
        gpuntt::GPU_NTT_Modulus_Ordered_Inplace(
            ...
            (3 * (context_->bsk_modulus[input1.depth_] + current_decomp_count)),
            (context_->bsk_modulus[input1.depth_] + current_decomp_count),
            new_prime_bsk_locations + location5); // @company CipherFlow
        ...
        fast_floor<<<dim3((context_->n >> 8), 3, 1), 256, 0, stream>>>(
            ...
            context_->inv_punctured_prod_mod_base_array_->data() + location,
            context_->base_change_matrix_Bsk_->data() + location2, context_->inv_prod_q_mod_Bsk_->data() + location1,
            context_->inv_punctured_prod_mod_B_array_->data() + location3,
            context_->base_change_matrix_q_->data() + location4, context_->base_change_matrix_msk_->data() + location3,
            context_->inv_prod_B_mod_m_sk_->data() +  input1.depth_, context_->prod_B_mod_q_->data() + location, context_->n_power, current_decomp_count,
            context_->bsk_modulus[input1.depth_]); // @company CipherFlow
        HEONGPU_CUDA_CHECK(cudaGetLastError());
    ...
    {   
        ...
        {
            cipherplain_kernel<<<dim3((context_->n >> 8), current_decomp_count, 2), 256, 0, stream>>>(
                input1.data(), input2.data(), output_memory.data(),
                context_->modulus_->data(), context_->n_power, current_decomp_count); // @company CipherFlow
            HEONGPU_CUDA_CHECK(cudaGetLastError());
        }
        else
        {
            ...
            threshold_kernel<<<dim3((context_->n >> 8), current_decomp_count, 1), 256, 0, stream>>>(
                input2.data(), temp1_plain_mul, context_->modulus_->data(),
                context_->upper_halfincrement_->data(), context_->upper_threshold_, context_->n_power,
                current_decomp_count); // @company CipherFlow
            HEONGPU_CUDA_CHECK(cudaGetLastError());
            ...
            gpuntt::GPU_NTT_Inplace(temp1_plain_mul, context_->ntt_table_->data(),
            ...
            gpuntt::GPU_INTT_Inplace(output_memory.data(), context_->intt_table_->data(),
        ...
        const cudaStream_t stream)
    {
        ...
        cipher_broadcast_kernel<<<dim3((context_->n >> 8), context_->Q_size, 1), 256, 0, stream>>>(
            input1.data() + (context_->Q_size << (context_->n_power + 1)), temp1_relin,
            context_->modulus_->data(), context_->n_power, context_->Q_prime_size);
    ...
    {   
        ...
        DeviceVector<Data64> temp_relin(
            (context_->n * context_->Q_size * context_->Q_prime_size) + (2 * context_->n * context_->Q_prime_size), stream);
        Data64* temp1_relin = temp_relin.data();
        Data64* temp2_relin = temp1_relin + (context_->n * context_->Q_size * context_->Q_prime_size);
        ...
        base_conversion_DtoQtilde_relin_leveled_kernel<<<
            dim3((context_->n >> 8), context_->d_leveled->operator[](input1.depth_), 1), 256, 0,
            stream>>>(
            input1.data() + (current_decomp_count << (context_->n_power + 1)),
            temp1_relin, context_->modulus_->data(),
            context_->base_change_matrix_D_to_Qtilda_leveled->operator[](input1.depth_)
                .data(),
            context_->Mi_inv_D_to_Qtilda_leveled->operator[](input1.depth_).data(),
            context_->prod_D_to_Qtilda_leveled->operator[](input1.depth_).data(),
            context_->I_j_leveled->operator[](input1.depth_).data(),
            context_->I_location_leveled->operator[](input1.depth_).data(), context_->n_power,
            context_->d_leveled->operator[](input1.depth_), current_rns_mod_count,
            current_decomp_count, input1.depth_,
            context_->prime_location_leveled->data() + location); // @company CipherFlow 
        HEONGPU_CUDA_CHECK(cudaGetLastError());
        ...
        // TODO: make it efficient
        int iteration_count_1 = context_->d_leveled->operator[](input1.depth_) / 4; // @company CipherFlow 
        int iteration_count_2 = context_->d_leveled->operator[](input1.depth_) % 4; // @company CipherFlow 
        if (relin_key.storage_type_ == storage_type::DEVICE)
        {
        ...
        divide_round_lastq_extended_leveled_kernel<<<
            ...
            temp2_relin, temp1_relin, context_->modulus_->data(), context_->half_p_->data(),
            context_->half_mod_->data(), context_->last_q_modinv_->data(), context_->n_power,
            current_rns_mod_count, current_decomp_count, first_rns_mod_count,
            first_decomp_count, context_->P_size); // @company CipherFlow 
        HEONGPU_CUDA_CHECK(cudaGetLastError()); 

        addition<<<dim3((context_->n >> 8), current_decomp_count, 2), 256, 0, stream>>>(
            temp1_relin, input1.data(), input1.data(), context_->modulus_->data(),
            context_->n_power); // @company CipherFlow 
    ...
    {   
        ...
        int iteration_count_2 = context_->d_leveled->operator[](input1.depth_) % 4; 
        if (relin_key.storage_type_ == storage_type::DEVICE)
        {
            keyswitch_multiply_accumulate_leveled_method_II_kernel<<<
                dim3((context_->n >> 8), current_rns_mod_count, 1), 256, 0, stream>>>(
        ...
        Ciphertext<Scheme::BFV>& input1, const cudaStream_t stream)
    {
        ...
        move_cipher_leveled_kernel<<<
            ...
            input1.data(), temp2_rescale, context_->n_power, current_decomp_count - 1);
        
        HEONGPU_CUDA_CHECK(cudaGetLastError());

        divide_round_lastq_rescale_kernel<<<
            dim3((context_->n >> 8), current_decomp_count - 1, 2), 256, 0, stream>>>(
            temp1_rescale, temp2_rescale, input1.data(), context_->modulus_->data(),
        ...
        const cudaStream_t stream)
    {
        ...
        DeviceVector<Data64> temp_rotation((2 * context_->n * context_->Q_size) + (context_->n * context_->Q_size) +
                                               (2 * context_->n * context_->Q_prime_size * context_->d_leveled->operator[](0)) +
                                               (2 * context_->n * context_->Q_prime_size),
                                           stream); // @company CipherFlow 


        Data64* temp0_rotation = temp_rotation.data();
        Data64* temp1_rotation = temp0_rotation + (2 * context_->n * context_->Q_size);
        Data64* temp2_rotation = temp1_rotation + (context_->n * context_->Q_size);
        Data64* temp3_rotation = temp2_rotation + (2 * context_->n * context_->Q_prime_size * context_->d_leveled->operator[](0)); // @company CipherFlow 
        ...
        base_conversion_DtoQtilde_relin_leveled_kernel<<<
            dim3((context_->n >> 8), context_->d_leveled->operator[](input1.depth_), 1), 256, 0,
            stream>>>(
            input1.data() + (current_decomp_count << context_->n_power), temp2_rotation,
            context_->modulus_->data(),
            context_->base_change_matrix_D_to_Qtilda_leveled->operator[](input1.depth_)
                .data(),
            context_->Mi_inv_D_to_Qtilda_leveled->operator[](input1.depth_).data(),
        ...
        gpuntt::GPU_NTT_Modulus_Ordered_Inplace(
            temp2_rotation, context_->ntt_table_->data(), context_->modulus_->data(), cfg_ntt,
            context_->d_leveled->operator[](input1.depth_) * current_rns_mod_count,
            current_rns_mod_count, new_prime_locations + location); // @company CipherFlow 
        ...
        gpuntt::GPU_NTT_Modulus_Ordered_Inplace(
            temp3_rotation, context_->intt_table_->data(), context_->modulus_->data(), cfg_intt,
            2 * current_rns_mod_count, current_rns_mod_count,
            new_prime_locations + location); // @company CipherFlow 
        ...
        divide_round_lastq_permute_bfv_kernel<<<
            ...
            context_->modulus_->data(), context_->half_p_->data(), context_->half_mod_->data(),
            context_->last_q_modinv_->data(), galois_elt, context_->n_power, current_rns_mod_count,
            current_decomp_count, first_rns_mod_count, first_decomp_count,
            context_->P_size); // @company CipherFlow 
        HEONGPU_CUDA_CHECK(cudaGetLastError());
        ...
        Galoiskey<Scheme::BFV>& galois_key, const cudaStream_t stream)
    {
        ...
        DeviceVector<Data64> temp_rotation((2 * context_->n * context_->Q_size) + (context_->n * context_->Q_size) +
                                               (2 * context_->n * context_->Q_prime_size * context_->d_leveled->operator[](0)) +
                                               (2 * context_->n * context_->Q_prime_size), // @company CipherFlow 
                                           stream);
        ...
        divide_round_lastq_permute_bfv_kernel<<<
            ...
            temp3_rotation, temp0_rotation, output_memory.data(),
            context_->modulus_->data(), context_->half_p_->data(), context_->half_mod_->data(),
            context_->last_q_modinv_->data(), galoiselt, context_->n_power, current_rns_mod_count,
            current_decomp_count, first_rns_mod_count, first_decomp_count,
            context_->P_size); // @company CipherFlow 
        HEONGPU_CUDA_CHECK(cudaGetLastError());
        ...
        Switchkey<Scheme::BFV>& switch_key, const cudaStream_t stream)
    {
        ...
        DeviceVector<Data64> temp_rotation(
            (2 * context_->n * context_->Q_size) + (context_->n * context_->Q_size) +
        ...
        const ExecutionOptions& options)
    {
        ...
        addition_plain_bfv_poly<<<dim3((context_->n >> 8), context_->Q_size, 2), 256, 0,
                                  options.stream_>>>(
        ...
        Ciphertext<Scheme::BFV>& input1, const ExecutionOptions& options)
    {
        ...
        addition_plain_bfv_poly_inplace<<<dim3((context_->n >> 8), context_->Q_size, 1), 256, 0,
                                          options.stream_>>>(
