                ...
                output_storage_manager(
                    ...
                    [&](Relinkey<Scheme::BFV>& rk_)
                    {
                        DeviceVector<Data64> errors_a(
                            2 * ctx->Q_prime_size * ctx->d_leveled->operator[](0) * ctx->n, options.stream_); // @company CipherFlow
                        Data64* error_poly = errors_a.data();
                        Data64* a_poly = error_poly + (ctx->Q_prime_size * ctx->d_leveled->operator[](0) * ctx->n); // @company CipherFlow

                        RandomNumberGenerator::instance()
                            .modular_uniform_random_number_generation(
                                a_poly, ctx->modulus_->data(), ctx->n_power,
                                ctx->Q_prime_size, ctx->d_leveled->operator[](0), options.stream_); // @company CipherFlow

                        RandomNumberGenerator::instance()
                            .modular_gaussian_random_number_generation(
                                error_std_dev, error_poly, ctx->modulus_->data(),
                                ctx->n_power, ctx->Q_prime_size, ctx->d_leveled->operator[](0), options.stream_); // @company CipherFlow
                        ...
                        gpuntt::GPU_NTT_Inplace(
                            error_poly, ctx->ntt_table_->data(), ctx->modulus_->data(),
                            cfg_ntt, ctx->d_leveled->operator[](0) * ctx->Q_prime_size, ctx->Q_prime_size); // @company CipherFlow
                        HEONGPU_CUDA_CHECK(cudaGetLastError());
                        ...
                        relinkey_gen_II_kernel<<<dim3((ctx->n >> 8),
                            ...
                            ctx->modulus_->data(), ctx->factor_->data(), ctx->Sk_pair_leveled->operator[](0).data(),
                            ctx->n_power, ctx->Q_prime_size, ctx->d_leveled->operator[](0), ctx->Q_size, ctx->P_size); // @company CipherFlow
                        HEONGPU_CUDA_CHECK(cudaGetLastError());
        ...
        const ExecutionOptions& options)
    {
        ...
        input_storage_manager(
            ...
            [&](Secretkey<Scheme::BFV>& sk_)
            {
                ...
                Data64* error_poly = errors_a.data();
                Data64* a_poly =
                    error_poly + (ctx->Q_prime_size * ctx->d_leveled->operator[](0) * ctx->n); // @company CipherFlow
                ...
                if (!gk.customized)
                {
                    ...
                    for (auto& galois : gk.galois_elt)
                    {
                        ...
                        gpuntt::GPU_NTT_Inplace(
                            error_poly, ctx->ntt_table_->data(),
                            ctx->modulus_->data(), cfg_ntt,
                            ctx->d_leveled->operator[](0) * ctx->Q_prime_size, ctx->Q_prime_size); // @company CipherFlow
                        HEONGPU_CUDA_CHECK(cudaGetLastError());
                        ...
                        galoiskey_gen_II_kernel<<<dim3((ctx->n >> 8),
                                                  ...
                                                  256, 0, options.stream_>>>(
                            output_memory.data(), sk.data(), error_poly, a_poly,
                            ctx->modulus_->data(), ctx->factor_->data(), inv_galois,
                            ctx->Sk_pair_leveled->operator[](0).data(), ctx->n_power, ctx->Q_prime_size, ctx->d_leveled->operator[](0), // @company CipherFlow
                            ctx->Q_size, ctx->P_size);
                        HEONGPU_CUDA_CHECK(cudaGetLastError());
                    ...
                    RandomNumberGenerator::instance()
                        .modular_gaussian_random_number_generation(
                            error_std_dev, error_poly, ctx->modulus_->data(),
                            ctx->n_power, ctx->Q_prime_size, ctx->d_leveled->operator[](0),
                    ...
                    gpuntt::GPU_NTT_Inplace(error_poly, ctx->ntt_table_->data(),
                                            ctx->modulus_->data(), cfg_ntt,
                                            ctx->d_leveled->operator[](0) * ctx->Q_prime_size,
                                            ctx->Q_prime_size); // @company CipherFlow
                    HEONGPU_CUDA_CHECK(cudaGetLastError());
                    ...
                    galoiskey_gen_II_kernel<<<dim3((ctx->n >> 8),
                        ...
                        output_memory.data(), sk.data(), error_poly, a_poly,
                        ctx->modulus_->data(), ctx->factor_->data(),
                        gk.galois_elt_zero, ctx->Sk_pair_leveled->operator[](0).data(), ctx->n_power,
                        ctx->Q_prime_size, ctx->d_leveled->operator[](0), ctx->Q_size, // @company CipherFlow
                        ctx->P_size);
                    HEONGPU_CUDA_CHECK(cudaGetLastError());
