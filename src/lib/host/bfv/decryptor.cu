...
namespace heongpu
{
                                          ...
                                          const cudaStream_t stream)
    {
        ...
        int current_decomp_count = Q_size - ciphertext.depth_; // @company CipherFlow
        ...
        {
            gpuntt::GPU_NTT(ct1, ct1_temp, ctx->ntt_table_->data(), ctx->modulus_->data(),
        ...
        {
            sk_multiplication<<<dim3((n >> 8), current_decomp_count, 1), 256, 0, stream>>>(
                ct1, secret_key_.data(), ct1_temp, ctx->modulus_->data(), n_power,
        ...
        if (ciphertext.in_ntt_domain_)
        {
            ...
            gpuntt::GPU_INTT(ct0, ct0_temp, ctx->intt_table_->data(),
                            ctx->modulus_->data(), cfg_intt, current_decomp_count, current_decomp_count); // @company CipherFlow

            gpuntt::GPU_INTT_Inplace(ct1_temp, ctx->intt_table_->data(),
                                    ctx->modulus_->data(), cfg_intt, current_decomp_count, 
                                    current_decomp_count); // @company CipherFlow
        ...
        decryption_kernel<<<dim3((n >> 8), 1, 1), 256, 0, stream>>>(
            ...
            ctx->plain_modulus_, ctx->gamma_, ctx->Qi_t_->data() + location1, ctx->Qi_gamma_->data() + location1,
            ctx->Qi_inverse_->data() + location1, ctx->mulq_inv_t_->data() + ciphertext.depth_, ctx->mulq_inv_gamma_->data() + ciphertext.depth_, ctx->inv_gamma_,
            n_power, current_decomp_count); // @company CipherFlow
        HEONGPU_CUDA_CHECK(cudaGetLastError());
                                            ...
                                            const cudaStream_t stream)
    {
        ...
        gpuntt::GPU_NTT_Inplace(ct1, ctx->ntt_table_->data(), ctx->modulus_->data(),
        ...
        sk_multiplicationx3<<<dim3((n >> 8), current_decomp_count, 1), 256, 0, stream>>>(
            ct1, secret_key_.data(), ctx->modulus_->data(), n_power, current_decomp_count); // @company CipherFlow
        HEONGPU_CUDA_CHECK(cudaGetLastError());
        ...
        gpuntt::GPU_INTT_Inplace(ct1, ctx->intt_table_->data(), ctx->modulus_->data(),
        ...
        decryption_kernelx3<<<dim3((n >> 8), 1, 1), 256, 0, stream>>>(
            ct0, ct1, ct2, plaintext.data(), ctx->modulus_->data(), ctx->plain_modulus_,
            ctx->gamma_, ctx->Qi_t_->data() + location, ctx->Qi_gamma_->data() + location, ctx->Qi_inverse_->data()  + location,
            ctx->mulq_inv_t_->data() + ciphertext.depth_, ctx->mulq_inv_gamma_->data() + ciphertext.depth_, ctx->inv_gamma_, n_power, current_decomp_count); // @company CipherFlow
        HEONGPU_CUDA_CHECK(cudaGetLastError());
    }
