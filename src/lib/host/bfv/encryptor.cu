...
namespace heongpu
{
                                          ...
                                          const cudaStream_t stream)
    {
        ...
        gpuntt::GPU_INTT_Inplace(pk_u_poly, ctx->intt_table_->data(),
                                ctx->modulus_->data(), cfg_intt, 2 * Q_prime_size,
                                Q_prime_size);
        
        // @company CipherFlow begin ---
        int first_rns_mod_count = Q_prime_size;
        int current_rns_mod_count = Q_prime_size - ciphertext.depth_;

        int first_decomp_count = Q_size;
        ...
        enc_div_lastq_bfv_kernel<<<dim3((n >> 8), current_decomp_count, 2), 256, 0,
            ...
            ctx->modulus_->data(), ctx->half_p_->data(), ctx->half_mod_->data(),
            ctx->last_q_modinv_->data(), ctx->plain_modulus_, ctx->Q_mod_t_->data() + location, ctx->upper_threshold_,
            ctx->coeeff_div_plainmod_->data() + location, n_power, current_rns_mod_count, current_decomp_count, 
            first_rns_mod_count, first_decomp_count,
            P_size); // @company CipherFlow
        HEONGPU_CUDA_CHECK(cudaGetLastError());
