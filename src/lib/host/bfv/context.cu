...
namespace heongpu
{
        ...
        int P_modulus_size)
    {
            ...
            (poly_modulus_degree_specified_))
        {
            ...
            Qprime_mod_bit_sizes_ = Q_mod_bit_sizes_;
            Qprime_mod_bit_sizes_.insert(Qprime_mod_bit_sizes_.end(),
                                         P_mod_bit_sizes_.begin(),
                                         P_mod_bit_sizes_.end());
    ...
    // @company CipherFlow
    int HEContextImpl<Scheme::BFV>::calculate_bsk_modulus_size(int size)
    {
    ...
    std::vector<Data64> HEContextImpl<Scheme::BFV>::generate_base_change_matrix_msk(
    ...
    std::vector<Modulus64> HEContextImpl<Scheme::BFV>::generate_q_Bsk_merge_modulus(
        std::vector<Modulus64> primes, std::vector<Modulus64> bsk_mod, int size)
    {
        std::vector<Modulus64> q_Bsk_merge_modulus;
        for (int i = 0; i < size; i++)
