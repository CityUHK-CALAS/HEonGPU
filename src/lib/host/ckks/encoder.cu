...
// @company CipherFlow: thin wrapper packaging the gpufft::GPU_Special_FFT
// configuration for sparse-encoding callers. GPU-FFT now supports n_power
// in [4, 24] via the small-N kernel pair (Special_*Core_Small) so the
// previous host-side fallback for log_slots < 11 is gone.
static void special_ifft(Complex64* device_buf, int log_slots, double fix,
                         Complex64* gpu_roots, cudaStream_t stream)
{
    gpufft::fft_configuration<Float64> cfg{};
    cfg.n_power = log_slots;
    cfg.fft_type = gpufft::type::INVERSE;
    cfg.mod_inverse = Complex64(fix, 0.0);
    cfg.stream = stream;
    gpufft::GPU_Special_FFT(device_buf, gpu_roots, cfg, 1);
}

static void special_fft(Complex64* device_buf, int log_slots,
                        Complex64* gpu_roots, cudaStream_t stream)
{
    gpufft::fft_configuration<Float64> cfg{};
    cfg.n_power = log_slots;
    cfg.fft_type = gpufft::type::FORWARD;
    cfg.stream = stream;
    gpufft::GPU_Special_FFT(device_buf, gpu_roots, cfg, 1);
}
    ...
    __host__ HEEncoder<Scheme::CKKS>::HEEncoder(HEContext<Scheme::CKKS> context)
    {
        ...
        slot_count_ = ctx->slot_count; // @company CipherFlow
        gap_ = (ctx->n >> 1) / slot_count_; // @company CipherFlow
        log_slot_count_ = ctx->log_slot_count; // @company CipherFlow
        fft_length = ctx->n * 2;
        ...
        for (int i = 1; i < (context_->n >> 1); i++) // @company CipherFlow
        {
            rot_group.push_back((5 * rot_group[i - 1]) % fft_length);
        }

        std::vector<Complex64> new_ordered_root_tables(context_->n >> 1, Complex64(0)); // @company CipherFlow
        for (int logm = 1; logm <= context_->n_power - 1; ++logm) // @company CipherFlow
        {
        ...
        std::vector<Complex64> new_ordered_inverse_root_tables(context_->n >> 1, // @company CipherFlow
                                                               Complex64(0)); 
        for (int logm = context_->n_power - 1; logm > 0; logm--) // @company CipherFlow
        {
            int idx_mod = 1 << (logm + 2);
            int gap = fft_length / idx_mod;
        ...
        const double scale, const cudaStream_t stream)
    {
        ...
        DeviceVector<Data64> output_memory(slot_n * context_->Q_size, stream); // @company CipherFlow
        ...
        special_ifft(temp_complex.data(), log_slot_count_, fix,
                     special_ifft_roots_table_->data(), stream);
        ...
        gpuntt::GPU_NTT_Inplace(output_memory.data(), context_->ntt_table_slot_->data(), // @company CipherFlow
        ...
        if (gap_ > 1)
        {
            DeviceVector<Data64> expanded_output_memory(context_->n * context_->Q_size, stream);
            sparse_ntt_expand_kernel<<<dim3((context_->n >> 8), context_->Q_size, 1), 256, 0,
                                      stream>>>(expanded_output_memory.data(),
                                                output_memory.data(),
                                                log_slot_count_, context_->n_power,
                                                context_->Q_size);
            HEONGPU_CUDA_CHECK(cudaGetLastError());
        ...
        const double scale, const cudaStream_t stream)
    {
        ...
        special_ifft(message_gpu.data(), log_slot_count_, fix,
                     special_ifft_roots_table_->data(), stream);
                                            ...
                                            const cudaStream_t stream)
    {
        ...
        // Collapse to single-block when slot_n < 256 so we don't launch a
        // zero-block grid at small log_slots (slot_n = 2 << log_slot_count_).
        const dim3 ringt_grid(slot_n < 256 ? 1u : (unsigned) (slot_n >> 8),
                              (unsigned) current_decomp_count, 1);
        const int ringt_block = slot_n < 256 ? slot_n : 256;
        ringt_to_pt_kernel<<<ringt_grid, ringt_block, 0, stream>>>(
        ...
        gpuntt::GPU_NTT_Inplace(output_memory.data(), context_->ntt_table_slot_->data(),
                                context_->modulus_->data(), cfg_ntt,
                                         ...
                                         const cudaStream_t stream)
    {
        ...
        special_fft(temp_complex.data(), log_slot_count_,
                    special_fft_roots_table_->data(), stream);
                                         ...
                                         const cudaStream_t stream)
    {
        ...
        special_fft(message_gpu.data(), log_slot_count_,
                    special_fft_roots_table_->data(), stream);
        ...
        const double scale, const cudaStream_t stream)
    {
        ...
        int slot_n = 1 << log_slot_n;
        DeviceVector<Data64> output_memory(slot_n * context_->Q_size, stream);
        ...
        gpuntt::GPU_NTT_Inplace(output_memory.data(), context_->ntt_table_dslot_->data(),
                                context_->modulus_->data(), cfg_ntt, context_->Q_size, context_->Q_size);
        ...
        if (gap_ > 1)
        {
            ...
            sparse_ntt_expand_kernel<<<dim3((context_->n >> 8), context_->Q_size, 1), 256, 0,
                                      stream>>>(expanded_output_memory.data(),
                                                output_memory.data(),
                                                log_slot_count, context_->n_power,
                                                context_->Q_size);
            HEONGPU_CUDA_CHECK(cudaGetLastError());
            output_memory = std::move(expanded_output_memory);
        }
        ...
        const cudaStream_t stream)
    {
        ...
        int gap = (context_->n >> 1) / slot_count;

        DeviceVector<Complex64> message_gpu(slot_count, stream);
        DeviceVector<Data64> temp_plain(context_->n * current_modulus_count, stream);
        ...
        gpuntt::GPU_INTT(plain.data(), temp_plain.data(), context_->intt_table_->data(),
                         context_->modulus_->data(), cfg_intt, current_modulus_count,
        ...
        encode_kernel_compose<<<slot_kernel_grid(slot_count), slot_kernel_block(slot_count), 0, stream>>>(
            message_gpu.data(), temp_plain.data(), context_->modulus_->data(),
            context_->Mi_inv_->data() + location1, context_->Mi_->data() + location2,
            context_->upper_half_threshold_->data() + location1,
            context_->decryption_modulus_->data() + location1, current_modulus_count,
            plain.scale_, two_pow_64, reverse_order_local.data(),
            context_->n_power, gap);
        HEONGPU_CUDA_CHECK(cudaGetLastError());
